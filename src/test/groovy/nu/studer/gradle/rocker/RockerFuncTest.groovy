...
rocker {
  foo {
    // `stripTimestamps` defaults to false
...
"""
    ...
    void "only generates exactly the same output for the same input when stripTimestamps=true (was #stripTimestamps), regardless of the last-modified time of the template"() {
        given:
        template('src/rocker/main/Example.rocker.html')
...
"""
        ...
        (contentSecondRun == contentFirstRun) == stripTimestamps
        contentSecondRun.contains('static public final long MODIFIED_AT =') == !stripTimestamps

        where:
        stripTimestamps << [true, false]
    }
